...
def get_image_blob(sample_a, sample_b, sample_length):
    im_blob = []
    for i in range(config.BATCH_SIZE):
        ...
        im_a = prep_im_for_blob_with_bbox(im_a, bbox_a, sample_length[i])
        im_b = prep_im_for_blob_with_bbox(im_b, bbox_b, sample_length[i])
...
def get_image_blob_cache(sample_a, sample_b, sample_length, cache):
    im_blob = []
    for i in range(config.BATCH_SIZE):
        if sample_a[i]['img_path'] in cache.keys():
            im_a = cache[sample_a[i]['img_path']]
            im_b = cache[sample_b[i]['img_path']]
        else:
            im_a = cv_load_image(sample_a[i]['img_path'])
            im_b = cv_load_image(sample_b[i]['img_path'])
            cache[sample_a[i]['img_path']] = im_a
            cache[sample_b[i]['img_path']] = im_b
...
def prep_im_for_blob_with_bbox(im, bbox, BBOX_SCALE):
    ...
    if config.BBOX_SCALE_TYPE == 'SCALE':
        ...
        bbox[0] -= int(BBOX_SCALE)
        bbox[1] -= int(BBOX_SCALE)
        bbox[2] += int(BBOX_SCALE)
        bbox[3] += int(BBOX_SCALE)
    else:
        raise
