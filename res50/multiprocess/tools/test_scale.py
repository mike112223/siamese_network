...
model_weights = '../../merge_models/resnet50_v8_iter_360000.caffemodel'
...
with open('/home/zhuyanjia/py-faster-rcnn/siamese_shelf/res50/multiprocess/test/length_image_list_test_scale.txt') as f:
    data1 = [i.strip().split() for i in f]
with open('/home/zhuyanjia/py-faster-rcnn/siamese_shelf/res50/multiprocess/test/length_image_list_test_p_scale.txt') as f:
...
for j in range(blen):
		...
		if labels and 1 in labels:
			...
			if labels.index(1) != distances.index(min(distances)):
				for i in [labels.index(1), distances.index(min(distances))]:
					...
					cv2.imwrite('../bad_examples_s/%d-%s.jpg'%(pair_id, obj_id[i][0]), img_list[i][0])
					cv2.imwrite('../bad_examples_s/%d-%s-%.2f.jpg'%(pair_id, obj_id[i][1], distances[i]), img_list[i][1])
	...
	image1 = crop_image(object1, float(l))
	image2 = crop_image(object2, float(l))
