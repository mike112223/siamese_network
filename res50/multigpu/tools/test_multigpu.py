...
model_weights = '../../models/resnet50_multigpu_v0_iter_30000.caffemodel'
...
with open('/home/zhuyanjia/py-faster-rcnn/siamese_shelf/res50/multigpu/test/length_image_list_test.txt') as f:
    data1 = [i.strip().split() for i in f]
with open('/home/zhuyanjia/py-faster-rcnn/siamese_shelf/res50/multigpu/test/length_image_list_test_p.txt') as f:
	...
	if label == 'none':
		if labels and 1 in labels:
			...
			if labels.index(1) != distances.index(min(distances)):
				...
				#	cv2.imwrite('../bad_examples/%s-%d.jpg'%(obj_id[i][0],pair_id), img_list[i][0])
				#	cv2.imwrite('../bad_examples/%s-%d-%.2f.jpg'%(obj_id[i][1],pair_id,distances[i]), img_list[i][1])
